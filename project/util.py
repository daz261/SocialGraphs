...
def load_table_from_wiki(page_name):
    ...
    printout = {a.contents[0] : a["href"].replace("/wiki/", "") for a in soup.find_all('a', href=True) if a.text}
    #print(printout['Paradise Lost'])
    text_to_links = {a.contents[0]: a["href"].replace("/wiki/", "")
                     for a in soup.find_all('a', href=True)
                     if a.text and a["href"].startswith("/wiki")}
    df["Album link"] = df["Album"].apply(lambda album: printout.get(album))
    df["Artist link"] = df["Artist"].apply(lambda artist: printout.get(artist))
    return df
